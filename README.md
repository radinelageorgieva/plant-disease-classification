...
*
*
* This calculated a specific weight for each disease category based on the training samples, successfully preventing the network from developing a bias toward majority classes and allowing the model to achieve a high weighted average F1-score of 91% across both rare and common diseases. Furthermore, to handle technical dataset flaws, we developed a custom SafeImageFolder class. This software solution automatically intercepted runtime errors from corrupted image files and fetched the next working image, ensuring that the entire training and testing pipeline remained completely stable without requiring manual data purging.The model architecture was optimized using a precise two-phase transfer learning approach. In Phase 1, we utilized feature extraction by freezing all the deep convolutional layers of the pre-trained EfficientNet-B0 base model, replacing the default classifier head with a custom sequential architecture containing a Dropout layer with a probability of 0.2 for regularization and a Linear layer to map the 1,280 extracted features to our 38 output classes. This phase was trained using the Adam optimizer with a learning rate of 0.0001 and Cross-Entropy Loss, achieving a baseline validation accuracy plateau of 87.79% by epoch 6. To break past this performance limit, we initiated Phase 2 by unfreezing the final convolutional block, Block 8, of the EfficientNet-B0 architecture. We trained the model for 5 additional epochs using the AdamW optimizer with discriminative learning rates, applying a very small learning rate of 0.00001 for Block 8 to gently adapt the deep visual filters without causing catastrophic forgetting, and a learning rate of 0.0001 for the classifier head. This targeted fine-tuning strategy successfully broke the plateau and pushed the final overall test accuracy to 91%.The true engineering value of this project lies in our deep statistical and visual investigation of the model's failure modes. By computing a quantitative error pair ranking across the entire dataset, we shifted our focus from individual images to systematic bottlenecks. The normalized confusion matrix showed incredible diagonal strength and perfect 1.00 classification scores for highly distinct classes like Orange Haunglongbing and Corn Healthy. However, the quantitative ranking exposed a critical systematic limitation in fine-grained visual differentiation, showing that the model recorded 225 misclassifications between Corn Northern Leaf Blight and Corn Cercospora Leaf Spot. Our confidence analysis revealed that these mistakes fall into two distinct failure categories. First, high-confidence misclassifications, where the model predicts the wrong class with 80% to 95% confidence because the long brown spots on different corn leaves have a massive visual overlap in dataset textures, completely misleading the network. Second, low-confidence errors, where the model's confidence drops below 35% and the probability gap between the true and predicted class is minimal, meaning the model is simply guessing between two highly similar-looking leaf spots.To open the black box of the neural network and validate its reliability, we integrated a custom Explainable AI workflow using Grad-CAM. By tracking the gradients of the winning classes as they flow into the final convolutional feature layer, we generated visual 2D activation heatmaps overlayed back onto the de-normalized images using OpenCV. This qualitative analysis of the heatmaps provided the most critical insight of the project by revealing exactly how the model gets tricked. In success cases, the model accurately targets the actual biological sickness. However, in failure modes, the network relies on geometric shortcuts. For example, in Tomato Yellow Leaf Curl Virus and when confusing Tomato Healthy with Cherry Healthy, the bright red zone locks onto the sharp leaf tips, outlines, and silhouettes rather than the sick veins or yellow spots in the middle. In Grape Leaf Blight, Tomato Target Spot, and Tomato Septoria, the model completely ignores the leaf surface and focuses its attention on the background shadows, image corners, or the grey canvas.In summary, while a 91% accuracy proves that transfer learning with EfficientNet-B0 is highly effective for large-scale agricultural classification, our deep error analysis shows that the model is highly sensitive to background noise and leaf geometry. This project demonstrates that achieving high accuracy on a dataset does not mean the model is looking at the correct biological patterns. To transition this system from a laboratory prototype to a real-world, production-ready agricultural deployment, future work must incorporate automated background removal, image segmentation, or spatial attention masking to force the convolutional layers to look strictly at the inner leaf pathology.
---

## References

* **PlantVillage Dataset**: Mohanty, S. P., Hughes, D. P., & Salathé, M. (2016). Using Deep Learning for Image-Based Plant Disease Detection. *Frontiers in Plant Science*, 7, 1419. Dataset repository: [spMohanty/PlantVillage-Dataset](https://github.com).
* **PyTorch Library**: Paszke, A., Gross, S., Massa, F., Lerer, A., Bradbury, J., Chanan, G., ... & Chintala, S. (2019). PyTorch: An Imperative Style, High-Performance Deep Learning Library. *Advances in Neural Information Processing Systems*, 32. Documentation: [pytorch.org](https://pytorch.org).
* **EfficientNet CNN**: Tan, M., & Le, Q. (2019). EfficientNet: Rethinking Model Scaling for Convolutional Neural Networks. *International Conference on Machine Learning (ICML)*, arXiv:1905.11946.
* **Grad-CAM (XAI)**: Selvaraju, R. R., Cogswell, M., Das, A., Vedantam, R., Parikh, D., & Batra, D. (2017). Grad-CAM: Visual Explanations from Deep Networks via Gradient-based Localization. *IEEE International Conference on Computer Vision (ICCV)*, 618-626.
* **AdamW Optimizer**: Loshchilov, I., & Hutter, F. (2017). Decoupled Weight Decay Regularization. *International Conference on Learning Representations (ICLR)*, arXiv:1711.05101.
* **MLflow Platform**: Zaharia, M., Chen, A., Chow, A., Davidson, A., Ghodsi, A., Hamel, S., ... & Zumar, D. (2018). Accelerating the Machine Learning Lifecycle with MLflow. Documentation: [mlflow.org](https://mlflow.org).
...
* **MLflow Platform**: MLflow AI Platform Team. (2018). Accelerating the Machine Learning Lifecycle with MLflow. Python API Reference Documentation: [mlflow.org/docs](https://mlflow.org).
* **PyTorch Library**: PyTorch Development Team. Training a CNN Classifier (Deep Learning with PyTorch: A 60 Minute Blitz). Documentation: [pytorch.org/tutorials](https://pytorch.org).
* **Vision Inspection CNN**: Towards Data Science. Building a Vision Inspection CNN for an Industrial Application. Practical guide: [towardsdatascience.com](https://towardsdatascience.com).
